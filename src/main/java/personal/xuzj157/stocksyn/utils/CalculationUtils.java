...
public class CalculationUtils {
    ...
    public static double getSum(RandomUnitS randomUnit, SecondCalculationUnit second) {
        double sum = 0;
        sum = sum + randomUnit.getNetincGrowRate() * second.getNetincGrowRate();
        sum = sum + randomUnit.getMainbusiincome() * second.getMainbusiincome();
        sum = sum + randomUnit.getNetprofit() * second.getNetprofit();
    ...
    public static List<RandomUnitS> getRandomS(Integer times) {
        List<RandomUnitS> randomUnitList = new LinkedList<>();
        for (int i = 0; i < times; i++) {
            randomUnitList.add(new RandomUnitS());
        }
        log.info("getRandom finish");
        return randomUnitList;
    }
